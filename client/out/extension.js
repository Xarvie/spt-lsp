...
function activate(context) {
    ...
    let serverCommand = config.get('serverPath', null);
    ...
    */
    ...
    const enableLog = logConfig.get('enableLog', true);
    const logPath = logConfig.get('logPath', '');
    // 构建服务器启动参数
    const serverArgs = [];
    if (enableLog) {
        serverArgs.push('--log');
    ...
    const serverOptions = {
        ...
        transport: node_1.TransportKind.stdio, // 使用标准输入/输出进行通信
        options: {
    ...
    client = new node_1.LanguageClient('sptscriptLsp', // 客户端的唯一 ID (内部使用)
