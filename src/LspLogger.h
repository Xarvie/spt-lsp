 ...
 */
inline std::string formatNode(const void* node, const char* kindStr, 
                               uint32_t beginLine, uint32_t beginCol, uint32_t beginOffset,
                               uint32_t endLine, uint32_t endCol, uint32_t endOffset) {
    std::ostringstream ss;
    ss << "Node{ptr=" << node 
       << ", kind=" << kindStr
       << ", range=[(" << beginLine << ":" << beginCol << " @" << beginOffset << ")"
       << " - (" << endLine << ":" << endCol << " @" << endOffset << ")]}";
    return ss.str();
}

inline std::string formatNodeSimple(const char* kindStr, 
                                     uint32_t beginOffset, uint32_t endOffset,
                                     bool isValid) {
    std::ostringstream ss;
    ss << kindStr << "[" << beginOffset << "-" << endOffset << "]"
       << (isValid ? "" : "(INVALID)");
    return ss.str();
}
...
#define LSP_LOG_NODE(node) do { \
    if (node) { \
        LSP_LOG(::lang::lsp::debug::formatNode( \
            node, \
            ::lang::ast::astKindToString((node)->kind), \
            (node)->range.begin.line, (node)->range.begin.column, (node)->range.begin.offset, \
            (node)->range.end.line, (node)->range.end.column, (node)->range.end.offset)); \
    } else { \
...
#define LSP_LOG_NODE_EX(label, node) do { \
    if (node) { \
        LSP_LOG(label << ": " << ::lang::lsp::debug::formatNodeSimple( \
            ::lang::ast::astKindToString((node)->kind), \
...
// Log offset search
#define LSP_LOG_SEARCH(offset, node) do { \
    if (node) { \
        LSP_LOG("Search offset=" << offset \
            << " in " << ::lang::ast::astKindToString((node)->kind) \
            << "[" << (node)->range.begin.offset << "-" << (node)->range.end.offset << "]" \
            << " valid=" << (node)->range.isValid() \
            << " contains=" << ((offset >= (node)->range.begin.offset && offset < (node)->range.end.offset) ? "YES" : "NO")); \
    } else { \
