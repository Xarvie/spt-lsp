...
std::any visitBlockStatement(LangParser::BlockStatementContext* ctx) override {
    if (!ctx) return static_cast<BlockStmtNode*>(factory_.makeBlockStmt(SourceRange::invalid(), {}));
...
std::any visitExpressionStmt(LangParser::ExpressionStmtContext* ctx) override {
    if (!ctx) return static_cast<Stmt*>(factory_.makeErrorStmt(SourceRange::invalid(), "missing expr"));
...
Expr* visitLValue(LangParser::LvalueContext* ctx) {
    if (!ctx) return factory_.makeErrorExpr(SourceRange::invalid(), "missing lvalue");
    ...
    if (baseCtx->IDENTIFIER()) {
        result = factory_.makeIdentifier(getRange(baseCtx->IDENTIFIER()), baseCtx->IDENTIFIER()->getText());
    } else {
...
std::any visitNormalAssignStmt(LangParser::NormalAssignStmtContext* ctx) override {
    if (!ctx) return static_cast<Stmt*>(factory_.makeErrorStmt(SourceRange::invalid(), "invalid assign"));
    
    auto range = getRange(ctx);
    std::vector<Expr*> targets;
    for (auto* lv : ctx->lvalue()) targets.push_back(visitLValue(lv));
    
    std::vector<Expr*> values;
    for (auto* e : ctx->expression()) values.push_back(expectExpr(e));
...
std::any visitUpdateAssignStmt(LangParser::UpdateAssignStmtContext* ctx) override {
    if (!ctx) return static_cast<Stmt*>(factory_.makeErrorStmt(SourceRange::invalid(), "invalid update"));
...
std::any visitIfStatement(LangParser::IfStatementContext* ctx) override {
    if (!ctx) return static_cast<Stmt*>(factory_.makeErrorStmt(SourceRange::invalid(), "invalid if"));
...
std::any visitWhileStatement(LangParser::WhileStatementContext* ctx) override {
    if (!ctx) return static_cast<Stmt*>(factory_.makeErrorStmt(SourceRange::invalid(), "invalid while"));
...
std::any visitForCStyleControl(LangParser::ForCStyleControlContext* ctx) override {
    if (!ctx) return static_cast<Stmt*>(factory_.makeErrorStmt(SourceRange::invalid(), "invalid for"));
...
std::any visitForEachExplicitControl(LangParser::ForEachExplicitControlContext* ctx) override {
    if (!ctx) return static_cast<Stmt*>(factory_.makeErrorStmt(SourceRange::invalid(), "invalid foreach"));
...
std::any visitForInitStatement(LangParser::ForInitStatementContext* ctx) override {
    if (!ctx) return static_cast<Stmt*>(factory_.makeEmptyStmt(SourceRange::invalid()));
...
std::any visitForUpdateSingle(LangParser::ForUpdateSingleContext* ctx) override {
    ...
    if (ctx->updateStatement()) return visit(ctx->updateStatement());
    if (ctx->assignStatement()) return visit(ctx->assignStatement());
    if (ctx->expression()) {
...
std::any visitReturnStmt(LangParser::ReturnStmtContext* ctx) override {
    if (!ctx) return static_cast<Stmt*>(factory_.makeReturnStmt(SourceRange::invalid()));
...
std::any visitDeferBlockStmt(LangParser::DeferBlockStmtContext* ctx) override {
    if (!ctx) return static_cast<Stmt*>(factory_.makeErrorStmt(SourceRange::invalid(), "invalid defer"));
    BlockStmtNode* body = expectBlock(ctx->blockStatement());
...
std::any visitImportNamespaceStmt(LangParser::ImportNamespaceStmtContext* ctx) override {
    if (!ctx) return static_cast<Stmt*>(factory_.makeErrorStmt(SourceRange::invalid(), "invalid import"));
...
std::any visitTypePrimitive(LangParser::TypePrimitiveContext* ctx) override {
    ...
    if (p->INT()) kind = PrimitiveKind::Int;
    else if (p->FLOAT()) kind = PrimitiveKind::Float;
    else if (p->NUMBER()) kind = PrimitiveKind::Number;
    else if (p->STRING()) kind = PrimitiveKind::String;
    else if (p->BOOL()) kind = PrimitiveKind::Bool;
    else if (p->VOID()) kind = PrimitiveKind::Void;
    else if (p->NULL_()) kind = PrimitiveKind::Null;
    else if (p->FIBER()) kind = PrimitiveKind::Fiber;
    else if (p->FUNCTION()) kind = PrimitiveKind::Function;
...
std::any visitTypeQualifiedIdentifier(LangParser::TypeQualifiedIdentifierContext* ctx) override {
    ...
    if (!qid) return static_cast<TypeNode*>(factory_.makeErrorType(getRange(ctx), "invalid type"));
...
std::any visitDeclaration(LangParser::DeclarationContext* ctx) override {
    if (!ctx) return static_cast<Decl*>(factory_.makeErrorDecl(SourceRange::invalid(), "missing decl"));
    
    NodeFlags exportFlag = ctx->EXPORT() ? NodeFlags::IsExport : NodeFlags::None;
    
    if (ctx->variableDeclaration()) {
        auto v = visit(ctx->variableDeclaration());
        if (auto* d = tryCastDecl(v)) { d->flags = d->flags | exportFlag; return v; }
    }
...
std::any visitDeclaration_item(LangParser::Declaration_itemContext* ctx) override {
    if (!ctx) return static_cast<VarDeclNode*>(factory_.makeVarDecl(SourceRange::invalid(), "", 
...
std::any visitVariableDeclarationDef(LangParser::VariableDeclarationDefContext* ctx) override {
    if (!ctx) return static_cast<Decl*>(factory_.makeErrorDecl(SourceRange::invalid(), "invalid var decl"));
    ...
    if (!type) type = factory_.makeErrorType(getRange(ctx), "missing type");
...
std::any visitMutiVariableDeclarationDef(LangParser::MutiVariableDeclarationDefContext* ctx) override {
    if (!ctx) return static_cast<Decl*>(factory_.makeErrorDecl(SourceRange::invalid(), "invalid mutivar"));
    ...
    for (auto* id : ctx->IDENTIFIER()) namesStorage.push_back(id->getText());
    
    // 转换为 string_view
    std::vector<std::string_view> names;
    for (const auto& s : namesStorage) names.push_back(s);
...
std::any visitMultiDeclaration(LangParser::MultiDeclarationContext* ctx) override {
    ...
    if (auto* vd = tryCast<VarDeclNode>(v)) {
        ...
        auto* decl = factory_.makeVarDecl(getRange(ctx), 
                                           factory_.strings().get(vd->name),
                                           vd->type, init);
...
std::any visitParameter(LangParser::ParameterContext* ctx) override {
    if (!ctx) return static_cast<ParameterDeclNode*>(factory_.makeParameterDecl(
        SourceRange::invalid(), "", factory_.makeErrorType(SourceRange::invalid(), "invalid param")));
...
std::any visitFunctionDeclarationDef(LangParser::FunctionDeclarationDefContext* ctx) override {
    if (!ctx) return static_cast<Decl*>(factory_.makeErrorDecl(SourceRange::invalid(), "invalid function"));
    ...
    return static_cast<Decl*>(factory_.makeFunctionDecl(
        range, name, retType, params, body, qualName, mods, false, hasVarArgs));
}

std::any visitMultiReturnFunctionDeclarationDef(LangParser::MultiReturnFunctionDeclarationDefContext* ctx) override {
    if (!ctx) return static_cast<Decl*>(factory_.makeErrorDecl(SourceRange::invalid(), "invalid function"));
    ...
    return static_cast<Decl*>(factory_.makeFunctionDecl(
        range, name, retType, params, body, qualName, mods, true, hasVarArgs));
}
...
std::any visitClassDeclarationDef(LangParser::ClassDeclarationDefContext* ctx) override {
    if (!ctx) return static_cast<Decl*>(factory_.makeErrorDecl(SourceRange::invalid(), "invalid class"));
...
std::any visitClassFieldMember(LangParser::ClassFieldMemberContext* ctx) override {
    if (!ctx) return static_cast<FieldDeclNode*>(factory_.makeFieldDecl(SourceRange::invalid(), "",
    ...
    return static_cast<FieldDeclNode*>(factory_.makeFieldDecl(getRange(ctx), name, type, init, mods));
}

std::any visitClassMethodMember(LangParser::ClassMethodMemberContext* ctx) override {
    if (!ctx) return static_cast<MethodDeclNode*>(factory_.makeMethodDecl(SourceRange::invalid(), "",
    ...
    return static_cast<MethodDeclNode*>(factory_.makeMethodDecl(getRange(ctx), name, retType, params, body, mods));
}

std::any visitMultiReturnClassMethodMember(LangParser::MultiReturnClassMethodMemberContext* ctx) override {
    if (!ctx) return static_cast<MethodDeclNode*>(factory_.makeMethodDecl(SourceRange::invalid(), "",
        factory_.makeErrorType(SourceRange::invalid(), "invalid method"), {},
        factory_.makeBlockStmt(SourceRange::invalid(), {})));
    ...
    return static_cast<MethodDeclNode*>(factory_.makeMethodDecl(getRange(ctx), name, retType, params, body, mods, true));
}
...
std::any visitCompilationUnit(LangParser::CompilationUnitContext* ctx) override {
    ...
    LSP_LOG("StringTable address=" << (void*)&factory_.strings() << ", size=" << factory_.strings().size());
    
    if (!ctx) {
        LSP_LOG("ctx is nullptr, returning empty CompilationUnit");
        return static_cast<Decl*>(factory_.makeCompilationUnit(
            SourceRange::invalid(), filename_, {}));
    }
    ...
    for (size_t i = 0; i < stmts.size(); ++i) {
        if (stmts[i]) {
            LSP_LOG("  stmts[" << i << "]: " << astKindToString(stmts[i]->kind)
                    << " range=[" << stmts[i]->range.begin.offset << "-" << stmts[i]->range.end.offset << "]");
        } else {
    ...
    LSP_LOG("Created CompilationUnit at address: " << (void*)result 
            << ", statements.size()=" << result->statements.size());
