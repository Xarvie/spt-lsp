...
public:
     ...
     * @return The symbol, or nullptr if not found
     */
    [[nodiscard]] Symbol* resolveWithScope(std::string_view name, 
                                            Scope*& foundInScope) const {
        Symbol* symbol = resolveLocal(name);
        if (symbol) {
    ...
    [[nodiscard]] bool isInsideLoop() const {
        return findEnclosingLoop() != nullptr;
    }
...
public:
     ...
     */
    [[nodiscard]] Scope* createScope(ScopeKind kind, Scope* parent,
     ...
     */
    [[nodiscard]] VariableSymbol* createVariable(
     ...
     */
    [[nodiscard]] ParameterSymbol* createParameter(
            std::string_view name,
            types::TypeRef type,
            ast::SourceLoc loc,
            uint32_t index) {
     ...
     */
    [[nodiscard]] FunctionSymbol* createFunction(
            std::string_view name,
            types::TypeRef type,
            ast::SourceLoc loc) {
     ...
     */
    [[nodiscard]] ClassSymbol* createClass(
            std::string_view name,
            ast::SourceLoc loc) {
     ...
     */
    [[nodiscard]] FieldSymbol* createField(
            std::string_view name,
            types::TypeRef type,
            ast::SourceLoc loc,
     ...
     */
    [[nodiscard]] MethodSymbol* createMethod(
            std::string_view name,
            types::TypeRef type,
            ast::SourceLoc loc,
            ClassSymbol* owningClass) {
     ...
     */
    [[nodiscard]] ImportSymbol* createImport(
            std::string_view name,
