...
public:
    ...
    [[nodiscard]] bool isPrimitive() const noexcept {
        return kind_ == TypeKind::Bool || kind_ == TypeKind::Int ||
               kind_ == TypeKind::Float || kind_ == TypeKind::String ||
               kind_ == TypeKind::Number;
    }
...
public:
    explicit ListType(TypeRef elementType)
        : Type(TypeKind::List), elementType_(elementType) {}
    ...
    [[nodiscard]] bool equals(const Type& other) const override {
        ...
        if (!elementType_ && !o.elementType_) return true;
        if (!elementType_ || !o.elementType_) return false;
...
public:
    ...
    [[nodiscard]] bool equals(const Type& other) const override {
        ...
        bool keysEqual = (!keyType_ && !o.keyType_) ||
                         (keyType_ && o.keyType_ && keyType_->equals(*o.keyType_));
    ...
    [[nodiscard]] size_t hash() const override {
        size_t h = Type::hash();
        if (keyType_) h ^= keyType_->hash() << 1;
        if (valueType_) h ^= valueType_->hash() << 2;
        return h;
    }
...
public:
    ...
    [[nodiscard]] const std::vector<TypeRef>& paramTypes() const noexcept { 
        return paramTypes_; 
    }
    ...
    [[nodiscard]] bool equals(const Type& other) const override {
        ...
        if (paramTypes_.size() != o.paramTypes_.size()) return false;
        if (isVariadic_ != o.isVariadic_) return false;
        
        for (size_t i = 0; i < paramTypes_.size(); ++i) {
            if (!paramTypes_[i] || !o.paramTypes_[i]) continue;
            if (!paramTypes_[i]->equals(*o.paramTypes_[i])) return false;
        }
        
        if (!returnType_ && !o.returnType_) return true;
        if (!returnType_ || !o.returnType_) return false;
    ...
    [[nodiscard]] size_t hash() const override {
        ...
        if (returnType_) h ^= returnType_->hash() << 3;
        return h;
    }
...
public:
    ...
    [[nodiscard]] const std::vector<TypeRef>& elementTypes() const noexcept {
    ...
    [[nodiscard]] bool equals(const Type& other) const override {
        ...
        if (elementTypes_.size() != o.elementTypes_.size()) return false;
        
        for (size_t i = 0; i < elementTypes_.size(); ++i) {
            if (!elementTypes_[i] || !o.elementTypes_[i]) continue;
            if (!elementTypes_[i]->equals(*o.elementTypes_[i])) return false;
        }
        return true;
    }
...
public:
    explicit ClassType(std::string name)
        : Type(TypeKind::Class), name_(std::move(name)) {}
...
public:
    ...
    [[nodiscard]] TypeRef makeFunctionType(std::vector<TypeRef> paramTypes,
                                           TypeRef returnType,
                                           bool isVariadic = false) {
        auto type = std::make_unique<FunctionType>(
            std::move(paramTypes), returnType, isVariadic);
        return internType(std::move(type));
    }
    ...
    [[nodiscard]] bool isAssignableTo(TypeRef source, TypeRef target) const {
        ...
        if (source->isNullType()) {
            return target->isList() || target->isMap() || 
                   target->isClass() || target->isFunction();
        }
    ...
    [[nodiscard]] TypeRef commonType(TypeRef a, TypeRef b) const {
        ...
        if (a->isAny()) return anyType_;
        if (b->isAny()) return anyType_;
     ...
     */
    [[nodiscard]] TypeRef getBinaryOpResultType(TypeRef left, TypeRef right,
                                                 ast::BinaryOp op) const {
